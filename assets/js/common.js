...
profiles via https://giscience.github.io/openrouteservice-r/reference/ors_profile.html
"driving-car"
"driving-hgv"
"cycling-regular"
"cycling-road"
...
https://api.openrouteservice.org/v2/directions/driving-car?api_key=5b3ce3597851110001cf624837bcd0c908f0494794652a5a7720f62a&start=42.681911,37.143611&end=28.966667,41.016667
...
var lineTypes = [
  {
    lineType: "wheels",
    aliases: ["bus", "minibus", "car", "taxi", "police car", "truck"],
  },
...
function StorySvg(id) {
  ...
  this.readDataAndInit = function () {
    var that = this;
    Promise.all([fetch(mapUrl), fetch(legsDataUrl), fetch(cacheFile)])
      ...
      .then(function (rawDatasets) {
        ...
        var legsDataStory = legsData.filter(
          (d) => parseInt(d.story_id) === parseInt(that.story_id)
        );
    ...
    this.projection = d3
      ...
      .fitExtent(
        [
          ...
          [this.width - this.margin * 2, this.height - this.margin * 2],
        ],
        lineString
      )
      .precision(0.1);
  ...
  this.addRoute = function (leg1, leg2, callback, routeOptions) {
    ...
    if (options.service === "openrouteservice") {
      ...
      const apiUrlPost =
        "https://api.openrouteservice.org/v2/directions/driving-car/geojson"; // post
      ...
      const requestId =
        "s" + this.story_id + "-l" + leg1.leg_id + "-l" + leg2.leg_id;
      ...
      const cacheKeyData = Object.assign(
        {},
        { requestUrl: apiUrlPost },
        requestData
      );
      ...
      fetch(apiUrlPost, {
        ...
        headers: {
          ...
          Authorization:
            "5b3ce3597851110001cf624837bcd0c908f0494794652a5a7720f62a",
        },
...
function cleanLegs(legs) {
  ...
  var clean = legs.filter((leg) => {
    ...
    var matchedItem = lineTypes.find((item) =>
      item.aliases.includes(leg.transport)
    );
